  ...
  socketApi.io.on("connect", (socket) => {
    ...
    if (addUser(socket.id, getNameByid(socket.id))) {
      ...
      socket.on("sendmeusers", (query) => {
        var check = `select * from exitero where user_name = '${getNameByid(
          socket.id
        )}'`;
        conn.query(check, (err1, docs2) => {
          ...
          if (docs2 != "") {
            if (docs2[0].room_name !== undefined) {
              var query4 = `select * from onlinetable where room_name = '${
                docs2[0].room_name ? docs2[0].room_name : ""
              }' order by time desc`;
      ...
      socket.on(
        "updateInfos",
        ({ nikename, username, namecolor, auth, bcs, cs }) => {
      ...
      socket.on("hanitmsg", ({ text }) => {
        ...
        conn.query(query, (err, docs) => {
          if (docs != "") {
            socketApi.io.emit("hanitMsgBack", {
              text: text,
      ...
      socket.on("sendhanitpost", ({ name, id, text, img }) => {
        var querys = `select * from person where user_name ='${getNameByid(
          socket.id
        )}'`;
      ...
      var Array = [false];
      socket.on("displeC", () => {
        var query = `update onlinetable set comm = 'yes' where name = '${getNameByid(
          socket.id
        )}'`;
        conn.query(query);
        Array[0] = true;
      ...
      socket.on("enbileC", () => {
        var query = `update onlinetable set comm = 'no' where name ='${getNameByid(
          socket.id
        )}'`;
        conn.query(query);
        Array[0] = false;
      ...
      socket.on("displeN", () => {
        var query = `update onlinetable set notification  = 'yes' where name = '${getNameByid(
          socket.id
        )}'`;
        conn.query(query);
        var query1 = `update onlinetable set  statscolor   = 'red' where name ='${getNameByid(
          socket.id
        )}'`;
        conn.query(query1);
        var query10 = `update exitero set  statscolor   = 'red' where user_name ='${getNameByid(
          socket.id
        )}'`;
        conn.query(query10);
        socketApi.io.emit("updateCard1");
      ...
      socket.on("enbileN", () => {
        var query = `update onlinetable set notification  = 'no' where name ='${getNameByid(
          socket.id
        )}'`;
        conn.query(query);
        var query1 = `update onlinetable set  statscolor   = 'green' where name ='${getNameByid(
          socket.id
        )}'`;
        conn.query(query1, (err, docs) => {
          if (err) console.log(err);
        });
        var query10 = `update exitero set  statscolor  = 'green' where user_name ='${getNameByid(
          socket.id
        )}'`;
        conn.query(query10);
        socketApi.io.emit("updateCard1");
        ...
        conn.query(query, (err, docs) => {
          ...
          if (docs != "") {
            if (docs[0].notification == "no") {
              ...
              socketApi.io
                .to(socket.id)
                .emit("backAlert", "هدا المستخدم غالق التنبيهات");
            }
          }
        });
      });
      ...
      console.log(
        "add room name " + ExiteRoomAddoOne(roomid, getStoreUser(socket.id))
      );
      ...
      socket.on("sendToRVB", ({ name, img, text, id }) => {
        socketApi.io
          .to(id)
          .emit("sendToRVBs", { name, text, img, id: socket.id });
      });
      ...
      socket.on("getPrvInfos", () => {
        ...
        conn.query(query, (err, docs) => {
          ...
          if (docs != "") {
            socketApi.io
              .to(socket.id)
              .emit("getPrvinfos", { name: docs[0].name, img: docs[0].img });
          }
        });
      });
          ...
          if (docsRom != "") {
            ...
            conn.query(query, (err, docs) => {
              if (err) console.log(err);
                ...
                var query1 = `select * from onlinetable where name = '${getNameByid(
                  socket.id
                )}'`;
                conn.query(query1, (err, docs) => {
                  if (err) console.log(err);
                  if (docs) {
                    socketApi.io.emit("leaveRoomIt", {
                      name: getNameByid(socket.id),
                      socket: socket.id,
                      img: docs[0].img,
                    });
                    var deletev = `delete from voice where user_id = '${getNameByid(
                      socket.id
                    )}'`;
                      ...
                      if (docsv) {
                        conn.query(getv, (errg, docsg) => {
                          if (docsg) {
                            socketApi.io
                              .to(socket.id)
                              .emit("updateVoiceLine", []);
                            socketApi.io
                              .to(docs[0].room_name)
                              .emit("updateVoiceLine", docsg);
      ...
      socket.on("roomid", ({ id }) => {
        console.log(id);
        var qu = `select * from onlinetable where name = '${getNameByid(
          socket.id
        )}'`;
              ...
              conn.query(query2, (err, docs) => {
                ...
                conn.query(query7, (err, docs) => {
                  if (err) console.log(err);
                    ...
                    socketApi.io.emit("rooms", { docs: docs });
                    socketApi.io.emit("userOrder", {
                      id: socket.id,
                      newRoom: id,
                    });
                    socketApi.io.to(socket.id).emit("userOrderN", "");
                    conn.query(
                      `delete from voice where user_id = '${getNameByid(
                        socket.id
                      )}'`,
                      (errde, docsde) => {
                        if (docsde) {
                          ...
                          conn.query(
                            `update onlinetable set room_name = '${id}' where user_id = '${socket.id}'`
                          );
        ...
        var query0 = `select * from onlinetable where name = '${getNameByid(
          socket.id
        )}'`;
        conn.query(query0, (err, docs1) => {
          if (err) console.log(err);
          if (docs1) {
            var addroom = `insert into exitero values('${getNameByid(
              socket.id
            )}' , '${id}' , '${socket.id}' , '${docs1[0].country}' , '${
              docs1[0].namecolor
            }' , '${docs1[0].namebg}' , '${docs1[0].img}')`;
            var addromUpdate = `update exitero set room_name = '${id}' , country = '${
              docs1[0].country
            }' , namecolor = '${docs1[0].namecolor}' , nameg = '${
              docs1[0].namebg
            }' , img = '${docs1[0].img}' where user_name = '${getNameByid(
              socket.id
            )}'; `;
            conn.query(addromUpdate, (err, docs) => {
              if (err) console.log(err);
              var updateon = `update onlinetable set room_name = '${id}' where name = '${getNameByid(
                socket.id
              )}'`;
          ...
          if (err) console.log(err);
            ...
            conn.query(query2, (err, docs) => {
              ...
              if (docs) {
                ...
                socketApi.io.emit("changeRoom", {
                  msg: `تم نقله الى غرفة أخرى`,
                  name: getStoreUser(),
                });
        ...
        socket.on("get-hanit-msg", () => {
          ...
          conn.query(query, (err, docs) => {
            ...
            if (docs) {
              socketApi.io
                .to(socket.id)
                .emit("back-hanit-msg", { docs: docs, socket: socket.id });
            }
          });
        });
        newone = false;
      ...
      socket.on("getCuurentUserRoom", () => {
        var ha = `select * from onlinetable where name = '${getNameByid(
          socket.id
        )}'`;
      ...
      socket.on(
        "sendmsg",
        ({ text, nikename, name, img, room, fontcolor, namebg, namecolor }) => {
          sendMsgTime = 0;
          var word = [];
          var send = true;
          var ing = "";
      ...
      socket.on("updatePersonalImg", (img) => {
        var query = `update person set img = '${img}' where user_name = '${getNameByid(
          socket.id
        )}'`;
        conn.query(query, (err, docs) => {
          if (err) console.log(err);
          if (docs) {
            var query1 = `update onlinetable set img = '${img}' where name = '${getNameByid(
              socket.id
            )}'`;
            conn.query(query1, (err, docs1) => {
              if (err) console.log(err);
              if (docs1) {
                var queryexi = `update exitero set img = '${img}' where user_name = '${getNameByid(
                  socket.id
                )}'`;
                conn.query(queryexi, (err, docs) => {
                  if (err) console.log(err);
                  if (docs) {
                    socketApi.io.emit("changeImgB", {
                      id: socket.id,
                      name: img,
                    });
                  }
      ...
      socket.on("addComment", ({ val, id }) => {
        ...
        conn.query(qu, (err1, docs1) => {
          ...
          values(${id} , '${val}' , '${getNameByid(socket.id)}' , 0 , '${
            docs1[0].img
          }')`;
          conn.query(query, (err, docs) => {
            if (err) console.log(err);
            if (docs) {
              ...
              conn.query(qq, (erq, docsq) => {
                ...
                if (docsq) {
                  var length;
                  if (docsq.length == 0) length = 0;
                  else length = docsq[0].len;
      ...
      socket.on("addHanitLikes", (id) => {
        ...
        conn.query(query, (err, docs) => {
          ...
          if (docs) {
            ...
            conn.query(query1, (err, docs1) => {
              socketApi.io.emit("updateHanitLike", {
                likes: docs1[0].likes,
                id: id,
              });
            });
      ...
      socket.on("join", ({ name }) => {
        ...
        socket.on("vToken", () => {
          if (allow) {
            socketApi.io
              .to(socket.id)
              .emit("sendToken", storeinBrowser.get("token"));
          }
        });
            ...
            if (docs) {
              ...
              conn.query(query2, (err2, docs2) => {
                ...
                if (docs2) {
                  ...
                  socketApi.io
                    .to(socket.id)
                    .emit("makeCall", { img: docs[0].img, name: docs[0].name });
                }
              });
            }
          });
        ...
        socket.on("removeVoice", ({ n, room }) => {
          var query = `delete from voice where user_id = '${getNameByid(
            socket.id
          )}'`;
          conn.query(query, (err, docs) => {
            if (err) console.log(err);
        ...
        conn.query(queryper, (err, docs1) => {
          ...
          if (docs1) {
            var addroom = `insert into exitero values('${getNameByid(
              socket.id
            )}' , 'الغرفة العامة رقم 1' , '${socket.id}' , '${
              docs1[0].country
            }' , '${docs1[0].nameColor}' , '${docs1[0].nameBC}' , '${
              docs1[0].img
            }' , 'عضو جديد' ,  '${docs1[0].nikname}' ,'green' , 0 )`;
        ...
        socket.on("hanitfile", ({ text }) => {
          ...
          var querys = `select * from onlinetable where name = '${getNameByid(
            socket.id
          )}'`;
          conn.query(querys, (err, docs) => {
            if (err) console.log(err);
            var query = `insert into hanitmsg values(${randomInt},'${result}', '${getNameByid(
              socket.id
            )}' ,0 , 0 , '${docs[0].img}', 0)`;
          ...
          var queryson = `select * from person where user_name = '${getNameByid(
            socket.id
          )}'`;
          conn.query(queryson, (err, docso) => {
            if (err) console.log(err);
            if (docso) {
              socketApi.io.emit("hanitFileBack", {
                text: text,
                name: getNameByid(socket.id),
                socket: socket.id,
                img: docso[0].img,
        ...
        conn.query(query, (err, docs) => {
                ...
                ( '${socket.id}' ,
                    '<div class="infos-users-oline">
                      ...
                      <div class="onlinetable-infos-person-we">
                        <p class="nameSow" style="font-wight:600;">${
                          drapo[0].nikname
                        }</p>
                        <p class="boi" >(${
                          drapo[0].auth ? drapo[0].auth : "حدث خطأ"
                        })</p></div>
                      </div>
                      <div class="drapo">
                      <img class="drapo-national" src="/flag/${
                    ...
                    conn.query(query2, (err, docs) => {
                      ...
                      conn.query(query3, (err3, docs3) => {
                        ...
                        if (docs3 != "") {
                          ...
                          var query = `select * from person where user_name ='${getNameByid(
                            socket.id
                          )}'`;
                          conn.query(query, (err, docs) => {
                            if (err) console.log(err);
                      ...
                      if (err) {
                        ...
                        var query3 =
                          "select divtag from onlinetable order by time desc";
                        conn.query(query3, (err, docs1) => {
                          if (err) console.log(err);
                          else if (docs1) {
                            var query = `select statscolor from onlinetable where name ='${getNameByid(
                              socket.id
                            )}'`;
                            conn.query(query, (err, docs) => {
                              if (err) console.log(err);
                              if (docs) {
                                ...
                                socketApi.io.emit("adduser", {
                                  docs: docs1,
                                  id: socket.id,
                                  sendMsgTime: docs[0].statscolor,
                                });
                              }
                            });
                          }
          ...
          socket.on("disconnect", () => {
            ...
            var queryVoice = `delete from voice where user_id = '${getNameByid(
              socket.id
            )}'`;
            conn.query(queryVoice, (err, docs) => {
              ...
              if (docs) {
                ...
                conn.query(queryro, (errRo, docsRo) => {
                  if (errRo) console.log(errRo);
                  if (docsRo) {
                    var queryroom = `select * from onlinetable where name ='${getNameByid(
                      socket.id
                    )}'`;
            ...
            if (userRemove[0]) {
              var query = `delete from person where user_name = '${getNameByid(
                socket.id
              )}'`;
              conn.query(query);
            }
                  ...
                  setTimeout(() => {
                    conn.query(query6, (err, docs) => {
                      ...
                      if (docs) {
                        ...
                        // update state
                        var querys =
                          "select count(*) as length from onlinetable";
                        conn.query(querys, (err, docs) => {
                          if (docs) {
                            conn.query(
                              `update online set number = ${docs[0].length}`
                            );
                          }
                        });
                        var query4 =
                          "select * from onlinetable order by time desc";
                        conn.query(query4, (err, docs1) => {
                          socketApi.io
                            .to(socket.id)
                            .emit("adduser", { docs: docs1, id: socket.id });
                          if (err) console.log(err);
                          else if (docs1) {
                            socketApi.io
                              .to(socket.id)
